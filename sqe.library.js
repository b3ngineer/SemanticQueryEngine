function SemanticQueryEngine() {
  var EQUAL = 0, GREATER_THAN_OR_EQUAL = 1, GREATER_THAN = 2, LESS_THAN_OR_EQUAL = -1, LESS_THAN = -2;
  var OPERATORS = {equal:EQUAL, greaterThanOrEqual:GREATER_THAN_OR_EQUAL, greaterThan:GREATER_THAN, lessThanOrEqual:LESS_THAN_OR_EQUAL, lessThan:LESS_THAN};
  ...
  SearchModel.prototype._numberExists = function(term) {
    var numbers = this._terms[0].length, low = 0, high = numbers - 1, mid = Math.floor(numbers / 2);
    while(high >= low) {
  ...
  SearchModel.prototype.addTerm = function(term) {
    ...
    var word = term.toString(), l = word.length;
    ...
    var num = this._terms[l].length / l, terms = [word];
    for(var i = 0;i < num;i++) {
  ...
  SearchModel.prototype.addRules = function(rules) {
    ...
    for(var i = 0, rulesLength = rules.length;i < rulesLength;i++) {
      ...
      for(var j = 0;j < ruleTermsLength;j++) {
        ...
        if(typeof rules[i].operators !== "undefined" && typeof rules[i].operators[j] != "undefined") {
  ...
  SearchModel.prototype.getTermId = function(term) {
    ...
    var w = term.toString(), l = w.length;
    if(!this._terms[l]) {
      return false
    }
    var terms = this._terms[l].length / l, low = 0, high = terms - 1, mid = Math.floor(terms / 2);
    while(high >= low) {
      var idx = l * mid;
